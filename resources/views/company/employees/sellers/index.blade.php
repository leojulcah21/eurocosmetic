<x-app-layout>
    ...
    {{-- Aquí va el x-data principal --}}
    <div x-data="{ openDeleteModal: false, itemIdToDelete: null }"
        @open-delete-modal.window="openDeleteModal = true; itemIdToDelete = $event.detail.id"
            ...
            <div class="relative w-full p-12 overflow-hidden bg-white shadow-md sm:rounded-lg">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-stone-700">
                        <thead class="text-xs font-bold uppercase text-stone-800 bg-stone-50">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-4">Cargo</th>
                                <th scope="col" class="px-4 py-4">Experiencia</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b">
                                ...
                                <td class="flex items-center justify-end px-4 py-3" x-data="{ open: false }">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-transition
                                        class="absolute z-10 mt-2 bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-700 dark:divide-gray-600"
                                        x-cloak>
                                        <ul class="py-1 text-sm" aria-labelledby="dropdown-button">
                                            <li>
                                                <button type="button"
                                                    onclick="window.location='{{ route('company.employees.sellers.edit', $seller) }}'"
                                            ...
                                            <li>
                                                <button type="button"
                                                    @click="$dispatch('open-delete-modal', { id: {{ $seller->id }} }); open = false"
                            ...
                            @empty
                            <tr>
                                <td colspan="8" class="px-4 py-3 text-center text-stone-800">
        ...
        <div x-show="openDeleteModal" x-transition x-cloak
            class="fixed inset-0 z-[1] flex items-center justify-center bg-black bg-opacity-50">
            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <form x-bind:action="`{{ url('company/employees/sellers') }}/${itemIdToDelete}`" method="POST"
                    class="flex items-center justify-center mt-4 space-x-4">
                    @csrf
                    @method('DELETE')

                    <x-danger-button type="button" @click="openDeleteModal = false">
                        {{ __('No, cancel') }}
                    </x-danger-button>
...
<script>
    Swal.fire({
        ...
        customClass: {
            ...
            title: 'text-green-700 text-xl font-semibold',
        },
    });
</script>
@endif
...
<script>
    Swal.fire({
        ...
        customClass: {
            ...
            title: 'text-red-700 text-xl font-semibold',
            htmlContainer: 'text-base text-red-600',
        },
    });
</script>
@endif
