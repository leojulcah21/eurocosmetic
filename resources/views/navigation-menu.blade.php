...
<aside
    ...
    <!-- Header -->
    <div class="h-[76px] flex items-center justify-between px-4">
        <!-- Logo -->
        <a href="{{ route('dashboard') }}" class='block text-sm text-white whitespace-nowrap'>
            <x-dashboard-logo class="w-40 text-white" />
        </a>

        <!-- Botón menú (solo visible en pantallas pequeñas) -->
        <button class="p-2 text-white xl:hidden" @click="open = !open">
            <x-icon name='menu' class="w-6 h-6" />
        </button>
    </div>

    <!-- Separador -->
    <div class="h-px mt-0 bg-transparent bg-gradient-to-r from-transparent via-white to-transparent"></div>

    <!-- Contenido del menú -->
    <div
        class="items-center block w-auto max-h-screen overflow-auto h-[calc(100vh-360px)] grow basis-full mt-2 transition-transform duration-300 ease-in-out"
        :class="{'translate-x-0': open, '-translate-x-full xl:translate-x-0': !open }">
